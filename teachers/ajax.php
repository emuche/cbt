...
if (Input::get('dept') && Input::get('level2') && Input::get('term2')) {
	...
	$subjects		= $db->get('teachers', array('user_id', '=', $_SESSION['user']))->all();
...
<?php		
	foreach ($subjects as $subject) {
		$all_subjects	= $db->action('SELECT * ', 'subject', array('id', '=', $subject->subject_id), array('AND', 'session_id', '=', $term))->all();
		foreach ($all_subjects as $all_subject) {
			echo	'<option value="'.$all_subject->id.'">'.Character::organize($all_subject->subject_name).'</option>';
		}
